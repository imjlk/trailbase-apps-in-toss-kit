...
- The repository root may pin `@apps-in-toss/framework` as a dev dependency for
  lockfile/reference and SDK type validation, but published/private kit packages must keep
...
- Apps in Toss SDK 3.x currently applies to WebView projects through
  `@apps-in-toss/web-framework@3.4.0`; it is not an upgrade target for this
...
snapshot script is intentionally detection-only. `packages-snapshot.json` records
discovered npm versions; these markers record reviewed references. Use
`bun run apps-in-toss:tracking:check` to enforce agreement between the root
`package.json` and English/Korean references. New discoveries are advisory and do
not automatically raise the reviewed version or fail a detection-only PR.
...
Document hashes and npm discovery are saved independently. If either collection
fails, its last valid snapshot is retained while the successful collection is
saved. The workflow can open the resulting PR and still reports the collection
failure. Identical results preserve their previous fetched timestamp.

The snapshots store document hashes and npm reference package metadata. It does
