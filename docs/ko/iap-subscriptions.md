...
주문 행의 존재만으로 소유권이 검증되지는 않습니다. 구독 권한의 적용과 조회에는
PAYMENT_COMPLETED, PURCHASED 또는 REFUNDED 제공자 상태로 확인된 결제·환불 원장이
필요합니다. 미검증 기존 행의 콜백은 RECEIVED로 유지하며, 이전에 생성된 권한도 조회되지
않습니다. 기존 소유권 충돌은 소유자 범위를 제한한 신뢰할 수 있는 조회와 소비 앱의
감사 가능한 복구 절차로 해결하세요. 웹훅은 주문 소유자를 변경하지 않습니다. 등록 확인
콜백은 eventVersion을 생략할 수 있지만, 명시하면 지원 문자열 `1.0`이어야 합니다.
