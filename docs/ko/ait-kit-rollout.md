...
- 자동 down 마이그레이션이 없습니다(추가된 것도 없음). 새 기준선이 기록한
  ...
  새 guest를 다시 적용하세요.
- 되돌리기 전에 0.11 guest가 살아 있는 동안 프로모션 작업을 모두 소진·정산
  하세요. 0.11 이전 guest는 저장된 거래 키를 이어서 처리할 수 없습니다.
  - `PREPARED` 행(키 저장 후 실행 시작 전 — 예: 키 커밋 직후 발송 중지)은
    지금 클레임하고 실행해 마무리해야 합니다.
  - 저장된 거래 키가 있는 `EXECUTING` 또는 UNKNOWN 행은 그 키로 status
    조회해 정산합니다.
  - 키 없는 UNKNOWN 행(저장 전에 응답을 잃은 레거시 단일 호출 grant)은
    status 엔드포인트를 쓸 수 없으므로 제공자·운영자 정산으로 다룹니다.
