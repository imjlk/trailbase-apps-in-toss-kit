...
보정하며, 새 `_user`를 insert할 때만 password hash를 생성합니다. 호환용
`upsert_verified_auth_user_tx`도 같은 경로로 위임하므로 기존 호출자도 일반적인 기존 사용자
bootstrap 경로에서 반복적인 password hash 작업을 피할 수 있습니다.
