...
기본 정규화기는 모순된 claim 응답을 거부합니다. 명시된 상태와 성공 플래그가 서로 맞아야 하며,
응답에 명시된 `campaignId`는 요청한 캠페인과 일치해야 합니다. 호환성을 위해 응답에 캠페인 ID가
없을 때는 요청한 ID를 사용할 수 있습니다. `normalizeResponse`를 직접 주입한 소비자는 기본
정규화기를 거치지 않으므로 자신의 응답 정합성 검사를 책임집니다.
