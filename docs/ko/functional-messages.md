...
두 메시지 등록 경로는 저장 전에 payload 객체와 배열에서 원본 수신자 예약 필드
(`tossUserKey`, `userKey`, `anonKey`)를 재귀적으로 제거합니다. 식별자는 비공개
HMAC/sealed 열에만 보관하세요. 기존 payload에 해당 값이 있으면 소비 앱의 마이그레이션으로
정리해야 합니다. 새 등록 요청은 멱등성 충돌 시 과거 행을 다시 쓰지 않습니다.
발송 payload도 같은 재귀 제거 함수를 거친 후 선택한 수신자를 추가합니다.
예약 필드 이름은 대소문자를 구분합니다. 소비 앱은 다른 메시지 context 필드나
인코딩된 문자열에도 원본 식별자를 복사하지 않아야 합니다.
